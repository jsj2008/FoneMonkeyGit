...
@synthesize FMWebViewController = FMWebViewController_;

- (id)initWithId:(NSString *)elementId
      andSession:(Session*)session
        andFMWebViewController:(FMWebViewController *)webVc
...
andCommandId:(NSString *)commandId{
  return [[[Element alloc] initWithId:elementId
                           andSession:session
           andFMWebViewController:webVc andCommandId:commandId] autorelease];
}
...
//        [FMWebViewController_ runNextCommand];
}
...
//                                 CGRectGetMidY(currentPosition));
    //[[FMWebViewController sharedInstance] selenium];
