...
- (id)init;
{
    if ((self = [super init]))
    {
        ...
        fileName = [NSString stringWithFormat:@"FM_LOG-%@.xml",[FMUtils timeStamp]];
    }
    return self;
}
...
- (void)writeResultFile;
{
    if (self.document && (getenv("FM_ENABLE_XML_REPORT") && strcmp(getenv("FM_ENABLE_XML_REPORT"),"NO") && !getenv("FM_ENABLE_QUNIT")))
    {
        NSLog(@"XML Output: %@",[FMUtils scriptPathForFilename:fileName]);
        
        [[self.document XMLData] writeToFile:[FMUtils scriptPathForFilename:fileName] atomically:NO];
    }
}
